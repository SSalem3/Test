        ...
        if self.data_type == "ip" or self.data_type == "domain":
            try:
                ...
                response_details = s.get(
                    "http://ip-api.com/json/{}?fields=status,message,country,regionName,city,isp,org,as,mobile,proxy,hosting".format(
                        data
                    )
                )
                if response_details.status_code == 200:
                    ...
                    self.error(
                        "Failed to query IP-API details. Status_code {}".format(
